...
- **No copyright/credit bar:** the footer's bottom bar ("©2026 … All Rights Reserved. Website By Pyxel Development", Elementor element `5c769d0`) is removed from every page. Reapply after refreshing the export.
- **Home page counters count up:** Simply Static didn't export Elementor's counter script, so the stats (90+ schools, 4 districts, 13,000+ students, 20+ years) were stuck at their starting values. `public/index.html` now holds the real totals and loads `public/site-scripts/counters.js`, which counts each one up from 0 when it scrolls into view (and skips the animation for visitors who prefer reduced motion). Reapply after refreshing the export.
- **Careers buttons go to AppOne:** on `/careers/`, the two hero buttons and each job card button read "Apply Now", and together with the Apply Now button on each job page they link to the Center's AppOne listings (https://jobs.appone.com/Sacramento-Chinese) instead of the site's application form. Reapply this after refreshing the export.
