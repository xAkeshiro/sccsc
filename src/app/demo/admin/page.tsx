...
export default async function AdminOverview() {
  ...
  const tiles = [
    { label: "New (submitted)", value: counts.submitted ?? 0, href: "/demo/admin/applications?status=submitted" },
    { label: "In review", value: counts.reviewing ?? 0, href: "/demo/admin/applications?status=reviewing" },
    { label: "Interviewing", value: counts.interview ?? 0, href: "/demo/admin/applications?status=interview" },
    { label: "Open postings", value: openJobs.n, href: "/demo/admin/jobs" },
  ];
    ...
    <div className="space-y-10">
      ...
      <section>
        <div className="flex items-center justify-between">
          ...
          <Link href="/demo/admin/applications" className="text-sm font-semibold text-brand-700 hover:underline">
            View all
          </Link>
        </div>
        <div className="mt-4 overflow-hidden rounded-2xl bg-white ring-1 ring-ink-100">
          {recent.length === 0 ? (
            ...
            <ul className="divide-y divide-ink-100">
              {recent.map((r) => (
                <li key={r.id}>
                  <Link href={`/demo/admin/applications/${r.id}`} className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 hover:bg-cream-50">
                    <span>
      ...
      <section>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {APPLICATION_STATUSES.map((s) => (
            <Link key={s} href={`/demo/admin/applications?status=${s}`} className="rounded-full bg-white px-4 py-2 text-sm ring-1 ring-ink-100 hover:ring-brand-200">
