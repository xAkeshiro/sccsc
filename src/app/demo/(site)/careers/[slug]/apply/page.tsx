...
export default async function ApplyPage({ params }: PageProps<"/demo/careers/[slug]/apply">) {
  ...
  if (!job) notFound();
  if (job.status !== "open") redirect(`/demo/careers/${slug}`);
  ...
  return (
    <Container className="py-12 sm:py-16">
      <Link href={`/demo/careers/${slug}`} className="inline-flex items-center gap-1.5 text-sm font-semibold text-brand-700 hover:underline">
      ...
      <div className="mx-auto mt-6 max-w-3xl">
        ...
        {existing ? (
          <div className="mt-8 rounded-3xl bg-white p-8 ring-1 ring-ink-100">
            ...
            <ButtonLink href="/demo/portal" className="mt-6">
              Go to my portal
            </ButtonLink>
          </div>
        ) : (
