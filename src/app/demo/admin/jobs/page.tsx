  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <ButtonLink href="/demo/admin/jobs/new">New posting</ButtonLink>
      </div>
        ...
        {rows.map((job) => (
          <li key={job.id} className="flex flex-wrap items-center justify-between gap-4 rounded-2xl bg-white p-5 ring-1 ring-ink-100">
            <div>
              ...
              <Link href={`/demo/admin/jobs/${job.id}`} className="mt-1 block text-lg font-bold hover:text-brand-700">
                {job.title}
              </Link>
              <Link href={`/demo/admin/applications?job=${job.id}`} className="text-sm font-semibold text-brand-700 hover:underline">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <ButtonLink href={`/demo/admin/jobs/${job.id}`} size="sm" variant="secondary">
                Edit
              </ButtonLink>
            </div>
          </li>
