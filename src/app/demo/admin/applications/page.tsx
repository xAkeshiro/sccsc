...
export default async function AdminApplications({ searchParams }: PageProps<"/demo/admin/applications">) {
  await requireAdmin();
  ...
  return (
    <div className="space-y-6">
      ...
      <form method="get" className="flex flex-wrap items-end gap-3 rounded-2xl bg-white p-4 ring-1 ring-ink-100">
        ...
        {(status || jobId) && (
          <Link href="/demo/admin/applications" className="px-2 py-2.5 text-sm font-semibold text-brand-700 underline">
            Clear
          </Link>
        )}
      </form>
            ...
            {rows.map((r) => (
              <tr key={r.id} className="hover:bg-cream-50">
                <td className="px-4 py-3">
                  <Link href={`/demo/admin/applications/${r.id}`} className="font-semibold text-ink-900 hover:text-brand-700">
                    {r.fullName}
                  </Link>
