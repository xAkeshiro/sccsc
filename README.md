...
- **Careers buttons go to AppOne:** on `/careers/`, the hero button and each job card button read "Apply Now", and together with the Apply Now button on each job page they link to the Center's AppOne listings (https://jobs.appone.com/Sacramento-Chinese) instead of the site's application form. Reapply this after refreshing the export.
